...
{
    /// <summary>
    /// Lớp Truy cập Dữ liệu (DAL) cho các bảng liên quan đến Tài liệu (tTaiLieu, tNhaXuatBan, tTacGia, tTaiLieu_TacGia,...)
    /// Sử dụng LINQ to SQL để thao tác với cơ sở dữ liệu.
    /// </summary>
    internal class QLTaiLieuDAL : DatabaseConnect
    {
        // ====================================================
        // I. PHƯƠNG THỨC TRUY VẤN DỮ LIỆU (GET/READ/SEARCH)
        // ====================================================

        /// <summary>
        /// Lấy tất cả thông tin chi tiết của Tài liệu, bao gồm cả tên của các khóa ngoại.
        /// </summary>
        /// <returns>Danh sách các đối tượng ẩn danh (dùng cho DataGridView) chứa thông tin Tài liệu đã được JOIN.</returns>
        ...
        /// <returns>Danh sách các đối tượng ẩn danh chứa Mã Tác giả, Tên Tác giả và Vai trò (dùng cho DGV phụ).</returns>
        ...
        /// <returns>Danh sách Tài liệu khớp với điều kiện lọc.</returns>
        public List<object> TimKiemTaiLieu(List<Filter> filters)
        {
            using (var db = new QLThuVienDataContext())
            {
                ...
                foreach (var filter in filters)
                {
                    ...
                    switch (filter.ColumnName)
                    {
                        ...
                        case "TenDinhDang":
                            ...
                            break;
                        case "NamXuatBan":
                            // Chuyển sang chuỗi để tìm kiếm một phần
                ...
                // 3. Kết quả cuối cùng (Chỉ lấy các trường cần thiết để trả về DataGridView)
        ...
        /// <summary>
        /// Cập nhật thông tin của một Tài liệu và danh sách Tác giả liên quan (thay thế hoàn toàn danh sách cũ).
        /// </summary>
        ...
        public bool SuaTaiLieu(tTaiLieu taiLieu, List<tTaiLieu_TacGia> danhSachTacGiaMoi, out string errorMessage)
        {
            ...
            try
            {
                using (var db = new QLThuVienDataContext())
                {
                    ...
                    // 2. Cập nhật Tác giả (Xóa tất cả cũ, Thêm tất cả mới)
        ...
        public bool XoaTaiLieu(string maTL, out string errorMessage)
        {
            ...
            try
            {
                using (var db = new QLThuVienDataContext())
                {
                    ...
                    // 1. Xóa Tác giả liên quan trước (để tránh lỗi khóa ngoại)
            ...
            // Bắt lỗi khóa ngoại nếu Tài liệu đang được tham chiếu bởi các bảng khác (VD: tPhieuMuon)
            catch (SqlException sqlex) when (sqlex.Number == 547)
            {
                errorMessage = "Không thể xóa tài liệu này vì nó đang được sử dụng trong các giao dịch hoặc bảng khác (lỗi khóa ngoại).";
                return false;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Lấy danh sách Tên đầy đủ Tác giả (HoDem + Ten) (dùng cho ComboBox/Filter).
        /// </summary>
        ...
        /// <summary>
        /// Lấy danh sách các Vai trò duy nhất của Tác giả trong liên kết Tài liệu (dùng cho ComboBox).
        /// </summary>
        ...
        /// <returns>Mã NXB hoặc null nếu không tìm thấy.</returns>
        ...
        /// <returns>Mã NN hoặc null nếu không tìm thấy.</returns>
        ...
        /// <returns>Mã ThL hoặc null nếu không tìm thấy.</returns>
        ...
        /// <returns>Mã DD hoặc null nếu không tìm thấy.</returns>
        ...
        /// <returns>Mã TG hoặc null nếu không tìm thấy.</returns>
